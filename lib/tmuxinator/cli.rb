module Tmuxinator
  class Cli < Thor
    ...
    desc "start [PROJECT] [SESSION_NAME]", "Start a tmux session using a project's tmuxinator config, with an optional custom SESSION_NAME for project reuse"
    map "s" => :start

    def start(name, custom_session=nil)
      project = Tmuxinator::Config.validate(name, custom_session)
    ...
    def debug(name, custom_session=nil)
      project = Tmuxinator::Config.validate(name, custom_session)
      puts project.render
    end

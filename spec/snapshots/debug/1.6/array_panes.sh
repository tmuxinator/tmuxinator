...
# Run on_project_start command.
  ...
  tmux -L interface set-option -t array-panes default-path /workspace/array-panes 1>/dev/null


  # Create windows.
  tmux -L interface new-window default-path /workspace/array-panes -k -t array-panes:0 -n shell
